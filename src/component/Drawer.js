import React from 'react'
import "../assets/css/drawer.scss"
import { Link } from 'react-router-dom';
import { useDispatch } from 'react-redux'
import { drawerFun } from '../redux/actions/action'

const Drawer = () => {

  const dispatch = useDispatch()

  const closeDrawer = () =>{
    dispatch(drawerFun())
  }

  return (
    <>
       <section class="drawer open" id="drawer" >
      <div class="drawer-bg">
      <svg className='desktop-svg' xmlns="http://www.w3.org/2000/svg" width="1514" height="1080" viewBox="0 0 1514 1080">
  <path id="Rectangle_117" data-name="Rectangle 117" d="M140,0H1514a0,0,0,0,1,0,0V944a136,136,0,0,1-136,136H940A940,940,0,0,1,0,140v0A140,140,0,0,1,140,0Z" fill="#f8f8f8"/>
</svg>

<svg className='mobile-svg' xmlns="http://www.w3.org/2000/svg" width="430" height="680" viewBox="0 0 430 680">
  <path id="Rectangle_180" data-name="Rectangle 180" d="M55.948,0H430a0,0,0,0,1,0,0V625.651A54.349,54.349,0,0,1,375.651,680h0A375.651,375.651,0,0,1,0,304.349V55.948A55.948,55.948,0,0,1,55.948,0Z" fill="#f8f8f8"/>
</svg>
      </div>
      <div class="drawer-contact-details">
       <div class="drawer-contact-container">
          <a href="tel:+91-9033-851-355" >Reach Out @ +91-9033-851-355</a>
          <a href="mailto:hello@nileshitsolution.com" >hello@nilesh<span>it</span>solution.com</a>
          <div class="social-icons">
          <a href="#" class="dribble" >
              <svg xmlns="http://www.w3.org/2000/svg"  version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 315.005 315.005" >
                <g id="XMLID_806_">
                  <path id="XMLID_810_" d="M157.502,315.005c86.846,0,157.5-70.659,157.5-157.51C315.002,70.652,244.348,0,157.502,0   S0.002,70.652,0.002,157.495C0.002,244.346,70.657,315.005,157.502,315.005z M157.502,278.32c-24.949,0-48.641-7.47-68.91-21.669   c8.266-13.713,33.391-48.075,87.209-68.604c13.566,36.779,20.475,68.164,23.279,82.886   C185.805,275.838,171.848,278.32,157.502,278.32z M234.223,250.697c-3.045-14.877-9.189-41.054-19.854-71.896   c5.393-0.537,10.916-0.808,16.516-0.808c19.547,0,35.857,3.27,44.469,5.419C269.602,209.694,255.018,233.486,234.223,250.697z    M256.364,88.678c12.102,17.326,19.482,37.668,21.311,58.722c-11.098-1.822-30.111-4.287-51.752-4.287   c-8.576,0-16.879,0.394-24.762,1.172c-0.207-0.494-0.416-0.99-0.627-1.488c-1.645-3.846-3.41-7.792-5.271-11.792   C227.418,116.581,246.903,98.915,256.364,88.678z M157.502,37.068c27.08,0,53.256,9.115,74.467,25.806   c-7.313,8.176-23.978,23.742-53.309,35.865c-14.623-26.151-29.629-48.322-38.209-60.465   C146.077,37.472,151.78,37.068,157.502,37.068z M100.67,51.327c7.076,9.965,22.684,32.586,38.441,59.674   c-42.383,10.219-79.625,12.437-97.402,12.869C50.77,92.999,72.155,66.631,100.67,51.327z M157.217,144.485   c1.373,2.775,2.689,5.51,3.949,8.194c-53.535,18.399-86.77,59.432-99.615,78.029c-15.254-20-23.875-44.212-24.625-69.414   C55.565,161.099,105.262,159.084,157.217,144.485z"/>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                <g>
                </g>
                </svg>
            </a>
            <a href="#" class="behance" >
            <svg fill="#000000" height="64px" width="64px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 512 512" ><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <path d="M344.1,244.5c-4.9-4.2-11-6.3-18.2-6.3c-7.8,0-13.9,2.2-18.3,6.7c-4.3,4.5-7,10.5-8.2,18.1h52.7 C351.7,254.9,348.9,248.8,344.1,244.5z"></path> <path d="M214.3,266.1c-3.8-1.7-9-2.6-15.8-2.7h-39v42.2H198c6.9,0,12.2-0.9,16-2.8c7-3.5,10.4-10,10.4-19.7 C224.4,274.9,221,269.2,214.3,266.1z"></path> <path d="M256,0C114.6,0,0,114.6,0,256s114.6,256,256,256s256-114.6,256-256S397.4,0,256,0z M291.6,182.3h67.8V202h-67.8 L291.6,182.3L291.6,182.3z M250.7,310.6c-3,4.9-6.7,9.1-11.2,12.4c-5,3.9-11,6.5-17.9,8c-6.9,1.4-14.3,2.1-22.4,2.1H128V174.9h76.5 c19.3,0.3,33,5.9,41,16.9c4.8,6.7,7.2,14.8,7.2,24.2c0,9.6-2.4,17.4-7.3,23.3c-2.7,3.3-6.8,6.3-12.1,9c8.1,3,14.2,7.6,18.3,14 s6.2,14.1,6.2,23.2C257.8,294.7,255.4,303.1,250.7,310.6z M384,283.2h-85.2c0.5,11.7,4.5,20,12.2,24.7c4.7,2.9,10.3,4.4,16.9,4.4 c6.9,0,12.6-1.8,17-5.4c2.4-1.9,4.5-4.6,6.3-8.1h31.2c-0.8,6.9-4.6,14-11.3,21.2c-10.5,11.4-25.1,17.1-44,17.1 c-15.6,0-29.3-4.8-41.2-14.4c-11.9-9.6-17.9-25.2-17.9-46.8c0-20.3,5.4-35.8,16.1-46.6c10.8-10.8,24.7-16.2,41.8-16.2 c10.2,0,19.4,1.8,27.5,5.5c8.2,3.6,14.9,9.4,20.2,17.3c4.8,6.9,7.9,15,9.3,24.2C383.8,265.4,384.1,273.1,384,283.2z"></path> <path d="M214.8,233.3c4.3-2.6,6.4-7.3,6.4-13.9c0-7.4-2.8-12.2-8.5-14.6c-4.9-1.6-11.1-2.5-18.7-2.5h-34.4v34.9H198 C204.9,237.3,210.4,236,214.8,233.3z"></path> </g> </g></svg>
              </a>
              <a href="#" class="linkdin" >
              <svg fill="#000000" height="64px" width="64px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"  viewBox="-143 145 512 512" ><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M113,145c-141.4,0-256,114.6-256,256s114.6,256,256,256s256-114.6,256-256S254.4,145,113,145z M41.4,508.1H-8.5V348.4h49.9 V508.1z M15.1,328.4h-0.4c-18.1,0-29.8-12.2-29.8-27.7c0-15.8,12.1-27.7,30.5-27.7c18.4,0,29.7,11.9,30.1,27.7 C45.6,316.1,33.9,328.4,15.1,328.4z M241,508.1h-56.6v-82.6c0-21.6-8.8-36.4-28.3-36.4c-14.9,0-23.2,10-27,19.6 c-1.4,3.4-1.2,8.2-1.2,13.1v86.3H71.8c0,0,0.7-146.4,0-159.7h56.1v25.1c3.3-11,21.2-26.6,49.8-26.6c35.5,0,63.3,23,63.3,72.4V508.1z "></path> </g></svg>
            </a>
            <a href="#" class="instagram" >
              <svg xmlns="http://www.w3.org/2000/svg" width="200px" height="200px" viewBox="0 0 24 24" fill="#fff" stroke="none" stroke-width="1">
                <g id="SVGRepo_bgCarrier"/> <path fill-rule="evenodd" clip-rule="evenodd" d="M7.46494 1.066C8.63828 1.01222 9.01228 1 12 1C14.9883 1 15.3617 1.01283 16.5344 1.066C17.7059 1.11917 18.5059 1.30556 19.2056 1.5775C19.9395 1.85381 20.6043 2.28674 21.1538 2.84617C21.7133 3.3956 22.1463 4.06046 22.4225 4.79439C22.6944 5.49411 22.8802 6.29406 22.934 7.46494C22.9878 8.63828 23 9.01228 23 12C23 14.9877 22.9872 15.3617 22.934 16.5351C22.8808 17.7059 22.6944 18.5059 22.4225 19.2056C22.1414 19.9286 21.7649 20.5427 21.1538 21.1538C20.6044 21.7133 19.9395 22.1463 19.2056 22.4225C18.5059 22.6944 17.7059 22.8802 16.5351 22.934C15.3617 22.9878 14.9877 23 12 23C9.01228 23 8.63828 22.9872 7.46494 22.934C6.29406 22.8808 5.49411 22.6944 4.79439 22.4225C4.07144 22.1414 3.45728 21.7649 2.84617 21.1538C2.28664 20.6044 1.85368 19.9395 1.5775 19.2056C1.30556 18.5059 1.11978 17.7059 1.066 16.5351C1.01222 15.3617 1 14.9883 1 12C1 9.01167 1.01283 8.63828 1.066 7.46556C1.11917 6.29406 1.30556 5.49411 1.5775 4.79439C1.85381 4.06051 2.28674 3.39568 2.84617 2.84617C3.39559 2.28664 4.06045 1.85368 4.79439 1.5775C5.49411 1.30556 6.29406 1.11978 7.46494 1.066ZM16.4452 3.046C15.2853 2.99344 14.937 2.98183 12 2.98183C9.063 2.98183 8.71467 2.99344 7.55478 3.046C6.48228 3.09489 5.89989 3.27394 5.51244 3.42489C4.99911 3.62411 4.63244 3.86244 4.24744 4.24744C3.86306 4.63244 3.62411 4.99911 3.42489 5.51244C3.27394 5.89989 3.09489 6.48228 3.046 7.55478C2.99344 8.71467 2.98183 9.063 2.98183 12C2.98183 14.937 2.99344 15.2853 3.046 16.4452C3.09489 17.5177 3.27394 18.1001 3.42489 18.4876C3.60111 18.9654 3.88219 19.3976 4.24744 19.7526C4.60234 20.1178 5.03461 20.3989 5.51244 20.5751C5.89989 20.7261 6.48228 20.9051 7.55478 20.954C8.71467 21.0066 9.06239 21.0182 12 21.0182C14.9376 21.0182 15.2853 21.0066 16.4452 20.954C17.5177 20.9051 18.1001 20.7261 18.4876 20.5751C19.0009 20.3759 19.3676 20.1376 19.7526 19.7526C20.1178 19.3977 20.3989 18.9654 20.5751 18.4876C20.7261 18.1001 20.9051 17.5177 20.954 16.4452C21.0066 15.2853 21.0182 14.937 21.0182 12C21.0182 9.063 21.0066 8.71467 20.954 7.55478C20.9051 6.48228 20.7261 5.89989 20.5751 5.51244C20.3759 4.99911 20.1376 4.63244 19.7526 4.24744C19.3676 3.86306 19.0009 3.62411 18.4876 3.42489C18.1001 3.27394 17.5177 3.09489 16.4452 3.046ZM10.5955 15.3909C11.0408 15.5754 11.518 15.6703 12 15.6703C12.9735 15.6703 13.907 15.2836 14.5953 14.5953C15.2837 13.907 15.6704 12.9734 15.6704 12C15.6704 11.0266 15.2837 10.093 14.5953 9.40468C13.907 8.71636 12.9735 8.32966 12 8.32966C11.518 8.32966 11.0408 8.4246 10.5955 8.60905C10.1501 8.7935 9.74553 9.06385 9.40471 9.40468C9.06389 9.7455 8.79353 10.1501 8.60908 10.5954C8.42463 11.0407 8.3297 11.518 8.3297 12C8.3297 12.482 8.42463 12.9593 8.60908 13.4046C8.79353 13.8499 9.06389 14.2545 9.40471 14.5953C9.74553 14.9361 10.1501 15.2065 10.5955 15.3909ZM8.00205 8.00201C9.06238 6.94168 10.5005 6.34599 12 6.34599C13.4996 6.34599 14.9377 6.94168 15.998 8.00201C17.0583 9.06234 17.654 10.5005 17.654 12C17.654 13.4995 17.0583 14.9376 15.998 15.998C14.9377 17.0583 13.4996 17.654 12 17.654C10.5005 17.654 9.06238 17.0583 8.00205 15.998C6.94172 14.9376 6.34603 13.4995 6.34603 12C6.34603 10.5005 6.94172 9.06234 8.00205 8.00201ZM18.9077 7.18838C19.1583 6.93773 19.2991 6.59779 19.2991 6.24333C19.2991 5.88886 19.1583 5.54892 18.9077 5.29828C18.657 5.04764 18.3171 4.90683 17.9626 4.90683C17.6082 4.90683 17.2682 5.04764 17.0176 5.29828C16.7669 5.54892 16.6261 5.88886 16.6261 6.24333C16.6261 6.59779 16.7669 6.93773 17.0176 7.18838C17.2682 7.43902 17.6082 7.57983 17.9626 7.57983C18.3171 7.57983 18.657 7.43902 18.9077 7.18838Z" fill="#fff"/> </svg>
            </a>
            <a href="#" class="whats-app">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="34"
                height="34"
                viewBox="0 0 34 34"
              >
                <g
                  id="Group_1182"
                  data-name="Group 1182"
                  transform="translate(-641 -735)"
                >
                  <g
                    id="Group_1181"
                    data-name="Group 1181"
                    transform="translate(73 48)"
                  >
                    <circle
                      id="Ellipse_15"
                      data-name="Ellipse 15"
                      cx="17"
                      cy="17"
                      r="17"
                      transform="translate(568 687)"
                    />
                  </g>
                  <path
                    id="Icon_simple-whatsapp"
                    data-name="Icon simple-whatsapp"
                    d="M16.7,13.727c-.287-.143-1.686-.827-1.947-.922s-.451-.143-.642.143-.736.92-.9,1.109-.333.2-.617.072A7.746,7.746,0,0,1,10.3,12.711a8.659,8.659,0,0,1-1.584-1.976c-.166-.286-.018-.444.124-.587s.287-.329.43-.5a2.419,2.419,0,0,0,.283-.473.525.525,0,0,0-.024-.5c-.072-.143-.641-1.546-.88-2.105s-.465-.487-.641-.487-.354-.014-.545-.014a1.076,1.076,0,0,0-.761.343,3.186,3.186,0,0,0-1,2.362,5.584,5.584,0,0,0,1.163,2.935,12.726,12.726,0,0,0,4.868,4.281c.681.286,1.212.458,1.626.6a3.949,3.949,0,0,0,1.794.115,2.937,2.937,0,0,0,1.924-1.361,2.363,2.363,0,0,0,.172-1.36c-.071-.129-.258-.2-.544-.329m-5.2,7.1H11.5a9.548,9.548,0,0,1-4.825-1.317l-.344-.2-3.579.931.959-3.479-.228-.358A9.466,9.466,0,0,1,18.223,4.654,9.342,9.342,0,0,1,21,11.325a9.478,9.478,0,0,1-9.482,9.435M19.585,3.292A11.589,11.589,0,0,0,11.5,0,11.348,11.348,0,0,0,1.619,17.025L0,22.906,6.046,21.33a11.512,11.512,0,0,0,5.45,1.381h0A11.4,11.4,0,0,0,22.9,11.357a11.257,11.257,0,0,0-3.336-8.028"
                    transform="translate(647 739.404)"
                    fill="#000"
                  />
                </g>
              </svg>
            </a>
            <a href="#" class="snap-chat">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="34"
                height="34"
                viewBox="0 0 34 34"
              >
                <g
                  id="Group_1183"
                  data-name="Group 1183"
                  transform="translate(-728 -747)"
                >
                  <g
                    id="Group_1181"
                    data-name="Group 1181"
                    transform="translate(160 60)"
                  >
                    <circle
                      id="Ellipse_15"
                      data-name="Ellipse 15"
                      cx="17"
                      cy="17"
                      r="17"
                      transform="translate(568 687)"
                    />
                  </g>
                  <path
                    id="Icon_metro-skype"
                    data-name="Icon metro-skype"
                    d="M23.219,14.229A9.943,9.943,0,0,0,11.753,3.114a5.941,5.941,0,0,0-8.226,8.211,9.906,9.906,0,0,0,9.831,11.464,10.172,10.172,0,0,0,1.812-.167,5.943,5.943,0,0,0,8.049-8.392Zm-8.93,5.954c-3.163.165-4.643-.535-6-1.808-1.514-1.421-.906-3.042.328-3.125s1.973,1.4,2.632,1.81,3.158,1.345,4.479-.165c1.439-1.644-.957-2.495-2.712-2.752-2.507-.371-5.672-1.727-5.425-4.4a4.592,4.592,0,0,1,4.4-4.233,7.379,7.379,0,0,1,5.876,1.6c1.615,1.377.741,2.917-.288,3.041s-2.177-2.265-4.437-2.3c-2.33-.037-3.9,2.425-1.028,3.124s5.96.986,7.069,3.617-1.726,5.425-4.891,5.59Z"
                    transform="translate(731.114 751.647)"
                    fill="#000"
                  />
                </g>
              </svg>
            </a>
          
           
           
            
           
           
          </div>
       </div>
      </div>
      <div class="drawer-list">
          <ul>
              <li><Link to={"/what-we-do"} onClick={closeDrawer} >What We Do</Link></li>
              <li><Link to={"/website-development"} onClick={closeDrawer} >agency | nits</Link></li>
              <li><Link to={"/our-work"} onClick={closeDrawer} >work | portfolio</Link></li>
              {/* <li><Link to={"/about"} onClick={closeDrawer} >About</Link></li> */}
              <li><Link to={"/releases"} onClick={closeDrawer} >releases</Link></li>
              <li><Link to={"/career"} onClick={closeDrawer} >career</Link></li>
              <li><Link to={"/blogs"} onClick={closeDrawer} >blogs</Link></li>
              <li><Link to={"/hire-us"} onClick={closeDrawer} >hire us</Link></li>
          </ul>
      </div>
  </section>
    </>
  )
}

export default Drawer